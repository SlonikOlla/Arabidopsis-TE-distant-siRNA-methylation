...
SRNA = {
    100: ROOT / "results/root_siRNA/root_24nt_100bp_matrix.tsv",
    500: ROOT / "results/root_siRNA/root_24nt_500bp_matrix.tsv",
}

METH = {
    ("CG",100):  ROOT / "results/root_methylation/root_CG_100bp_matrix.tsv",
    ("CG",500):  ROOT / "results/root_methylation/root_CG_500bp_matrix.tsv",
    ("CHG",100): ROOT / "results/root_methylation/root_CHG_100bp_matrix.tsv",
    ("CHG",500): ROOT / "results/root_methylation/root_CHG_500bp_matrix.tsv",
    ("CHH",100): ROOT / "results/root_methylation/root_CHH_100bp_matrix.tsv",
    ("CHH",500): ROOT / "results/root_methylation/root_CHH_500bp_matrix.tsv",
}
...
cache = OUT / f"phosphate_window_opportunity_cache_{args.feature}"
cache.mkdir(exist_ok=True)

genome_file = cache / "phosphate_chrom_sizes.txt"
...
def make_mapping(context, resolution, time):
    ...
    scol = "delta24_CPM" if resolution == 100 else "delta_CPM"

    s = pd.read_csv(
        SRNA[resolution],
        sep="\t",
        usecols=["chr","start","end",scol]
    )

    # Standardize the sRNA-difference column internally
    if scol != "delta24_CPM":
        s = s.rename(columns={scol: "delta24_CPM"})
...
outfile = OUT / (
    f"phosphate_{args.feature}_window_opportunity_GO_"
