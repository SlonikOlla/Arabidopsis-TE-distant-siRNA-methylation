...
SRNA = {
    100: ROOT / "pathogen_consensus/results/pathogen_24nt_100bp_matrix.tsv",
    500: ROOT / "pathogen_consensus/results/pathogen_24nt_500bp_matrix.tsv",
}

METH = {
    ("CG",100):  ROOT / "pathogen_consensus/results/pathogen_CG_100bp_matrix.tsv",
    ("CG",500):  ROOT / "pathogen_consensus/results/pathogen_CG_500bp_matrix.tsv",
    ("CHG",100): ROOT / "pathogen_consensus/results/pathogen_CHG_100bp_matrix.tsv",
    ("CHG",500): ROOT / "pathogen_consensus/results/pathogen_CHG_500bp_matrix.tsv",
    ("CHH",100): ROOT / "pathogen_consensus/results/pathogen_CHH_100bp_matrix.tsv",
    ("CHH",500): ROOT / "pathogen_consensus/results/pathogen_CHH_500bp_matrix.tsv",
}
...
FEATURE_BEDS = {
    "gene_body": ANN / "Araport11_genes.bed",
    "promoter":  ANN / "Araport11_promoters_1kb.bed",
}
...
parser.add_argument(
    ...
    choices=[1,5,10],
    default=1,
    help="Joint-tail percentage used to define candidate windows"
)
parser.add_argument("--seed", type=int, default=20260830)
parser.add_argument("--batch", type=int, default=200)
args = parser.parse_args()

GENES = FEATURE_BEDS[args.feature]

rng = np.random.default_rng(args.seed)

print(f"Feature: {args.feature}", flush=True)
...
cache = OUT / f"pathogen_window_opportunity_cache_{args.feature}"
cache.mkdir(exist_ok=True)

genome_file = cache / "pathogen_chrom_sizes.txt"
...
def make_mapping(context, resolution, time):
    ...
    if time == "6h":
        ...
        ev_col = "GSM491570_ev_6hpi"
    elif time == "14h":
        mock_cols = [
            "GSM491572_mock_14hpi_rep1",
            "GSM491573_mock_14hpi_rep2",
        ]
        ev_col = "GSM491576_ev_14hpi"
    else:
        raise ValueError(f"Unexpected pathogen time: {time}")

    s = pd.read_csv(
        SRNA[resolution],
        sep="\t",
        usecols=["chr","start","end"] + mock_cols + [ev_col]
    )

    for c in mock_cols + [ev_col]:
        s[c] = pd.to_numeric(s[c], errors="coerce")

    # Stress-associated 24-nt difference:
    # infected EV library minus mean of the two time-matched mock libraries.
    s["delta24_CPM"] = (
        s[ev_col] - s[mock_cols].mean(axis=1)
    )

    s = s[["chr","start","end","delta24_CPM"]]
...
cand = cand[
    (cand["stress"] == "pathogen") &
    (cand["feature"] == args.feature) &
    (cand["tail_pct"] == args.tail)
].copy()
...
for context in ["CG","CHG","CHH"]:
    ...
    for direction in ["gain","loss"]:
        ...
        for j,col in enumerate(test_cols):
            ...
            # Candidate genes responsible for this observed GO hit
            hit_genes = sorted(
                observed_genes &
                term_genes[go]
            )
...
if results:
    ...
    res = pd.DataFrame(columns=[
        "context","direction","GO_ID","GO_term",
        "observed_hits","observed_genes",
        "null_mean_hits","null_sd",
        "fold_vs_window_null","empirical_p",
        "null_ge_observed","n_observed_genes",
        "nperm","FDR_BH"
    ])

outfile = OUT / (
    f"pathogen_{args.feature}_window_opportunity_GO_"
    f"joint{args.tail:g}pct_{args.nperm}perm.tsv"
)
...
sigfile = OUT / (
    f"pathogen_{args.feature}_window_opportunity_GO_"
    f"joint{args.tail:g}pct_{args.nperm}perm_significant.tsv"
)
