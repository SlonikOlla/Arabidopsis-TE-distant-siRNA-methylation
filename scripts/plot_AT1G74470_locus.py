...
OUT = ROOT / "results/locus_plot_data/AT1G74470"
...
# Tight display around locus
XMIN = 27991000
XMAX = 27993500

# Recurrent windows
LOSS500_START = 27991500
LOSS500_END   = 27992000

CORE100_START = 27991600
CORE100_END   = 27991700
...
srna.to_csv(OUT / "AT1G74470_24nt_100bp.tsv", sep="\t", index=False)
cg.to_csv(OUT / "AT1G74470_CG_100bp.tsv", sep="\t", index=False)
chg.to_csv(OUT / "AT1G74470_CHG_100bp.tsv", sep="\t", index=False)
chh.to_csv(OUT / "AT1G74470_CHH_100bp.tsv", sep="\t", index=False)

def midpoint(df):
    return (df["start"] + df["end"]) / 2.0

# ------------------------------------------------------------
# Extract 500-bp methylation deltas
# ------------------------------------------------------------
def get_window_deltas(df, context):

    r = df[
        (df["chr"].astype(str) == str(CHR)) &
        (df["start"] == LOSS500_START) &
        (df["end"] == LOSS500_END)
    ]
...
# ------------------------------------------------------------
# Highlight 500-bp region + 100-bp core
# ------------------------------------------------------------
for ax in [ax_gene] + data_axes:

    # broader 500-bp recurrent region
    ax.axvspan(
        LOSS500_START,
        LOSS500_END,
        color="0.84",
...
ax_right.text(
    0.5, 0.795,
    f"Chr1:{LOSS500_START:,}–{LOSS500_END:,}",
    ha="center",
    fontsize=8.5,
    transform=ax_right.transAxes
)
...
# ------------------------------------------------------------
# Colored methylation legend
# ------------------------------------------------------------
handles = [
    Line2D([0], [0], color=COLORS["Ct"], marker="o",
           linewidth=1.5, markersize=5, label="Ct"),
    Line2D([0], [0], color=COLORS["6 h"], marker="o",
           linewidth=1.5, markersize=5, label="6 h"),
    Line2D([0], [0], color=COLORS["12 h"], marker="o",
           linewidth=1.5, markersize=5, label="12 h"),
    Line2D([0], [0], color=COLORS["24 h"], marker="o",
           linewidth=1.5, markersize=5, label="24 h"),
]
...
ax_right.text(
    0.5, 0.135,
    f"Chr1:{CORE100_START:,}–{CORE100_END:,}",
    ha="center",
    fontsize=8.5,
    transform=ax_right.transAxes
)
...
# ------------------------------------------------------------
fig.suptitle(
    "AT1G74470: heat-associated 24-nt siRNA and DNA methylation",
    fontsize=14,
    fontweight="bold",
    y=0.985
)

png = OUT / "AT1G74470_heat_locus_FINAL.png"
pdf = OUT / "AT1G74470_heat_locus_FINAL.pdf"
...
print("\n500-bp LOSS WINDOW")
print(f"Chr1:{LOSS500_START:,}-{LOSS500_END:,}")

for ctx in ["CG", "CHG", "CHH"]:
    print(ctx, [fmt(v) for v in loss[ctx]])

print("\nExact 100-bp recurrent core:")
print(f"Chr1:{CORE100_START:,}-{CORE100_END:,}")
print("12 h: CG+CHG+CHH")
