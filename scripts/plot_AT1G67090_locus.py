...
OUT = ROOT / "results/locus_plot_data/AT1G67090"
...
# Exact recurrent 500-bp methylation-loss window
LOSS_START = 25048500
LOSS_END   = 25049000
...
# Save plotting inputs
srna.to_csv(OUT / "AT1G67090_24nt_100bp.tsv", sep="\t", index=False)
cg.to_csv(OUT / "AT1G67090_CG_100bp.tsv", sep="\t", index=False)
chg.to_csv(OUT / "AT1G67090_CHG_100bp.tsv", sep="\t", index=False)
chh.to_csv(OUT / "AT1G67090_CHH_100bp.tsv", sep="\t", index=False)

def midpoint(df):
    return (df["start"] + df["end"]) / 2.0

# ============================================================
# Exact 500-bp delta extraction
# ============================================================
...
# ============================================================
# Gene model — MINUS STRAND, arrow points LEFT
# ============================================================
...
ax_gene.annotate(
    "",
    xy=(GENE_START, gene_y),
    xytext=(GENE_START + 250, gene_y),
    arrowprops=dict(
        arrowstyle="-|>",
        lw=2,
        mutation_scale=16
...
# ============================================================
fig.suptitle(
    "AT1G67090 (RBCS1A): heat-associated 24-nt siRNA and DNA methylation",
    fontsize=14,
    fontweight="bold",
    y=0.985
)

png = OUT / "AT1G67090_heat_locus_FINAL.png"
pdf = OUT / "AT1G67090_heat_locus_FINAL.pdf"
